...
import { LockKeyhole } from '@ocean/icons';
import { $t, useI18n } from '@ocean/locales';
import { storeToRefs, useAccessStore } from '@ocean/stores';

import { useScrollLock } from '@ocean-core/composables';
...
const [Form, { form, validate }] = useOceanForm(
  reactive({
    commonConfig: {
      hideLabel: true,
    ...
    <transition name="slide-right">
      <div v-if="showUnlockForm" class="flex-center size-full" @keydown.enter.prevent="handleSubmit">
        <div class="flex-col-center mb-10 w-[300px]">
          <OceanAvatar :src="avatar" class="enter-x mb-6 size-20" />

          <div class="enter-x mb-2 w-full items-center">
            <Form />
          </div>
          <OceanButton class="enter-x w-full" @click="handleSubmit">
            {{ $t('ui.widgets.lockScreen.entry') }}
          </OceanButton>
          <OceanButton class="enter-x my-2 w-full" variant="ghost" @click="$emit('toLogin')">
            {{ $t('ui.widgets.lockScreen.backToLogin') }}
          </OceanButton>
          <OceanButton class="enter-x mr-2 w-full" variant="ghost" @click="toggleUnlockForm">
            {{ $t('common.back') }}
          </OceanButton>
        </div>
      </div>
