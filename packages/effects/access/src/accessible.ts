...
import type { AccessModeType, GenerateMenuAndRoutesOptions, RouteRecordRaw } from '@ocean/types';
...
async function generateAccessible(mode: AccessModeType, options: GenerateMenuAndRoutesOptions) {
  ...
  accessibleRoutes.forEach((route) => {
    if (root && !route.meta?.noBasicLayout) {
      ...
      try {
        ...
        console.error(`Failed to add route: ${String(route.name)}:`, error?.message);
      }
    }
  });
  ...
  if (root) {
    ...
    try {
      ...
      console.error(`Failed to add route: ${String(root.name)}:`, error?.message);
    }
  }
