...
import type { ExtendedFormApi, OceanFormProps } from './types';

// import { toRaw, watch } from 'vue';
import { nextTick, onMounted, watch } from 'vue';

import { useForwardPriorityValues } from '@ocean-core/composables';
import { cloneDeep, get, isEqual, set } from '@ocean-core/shared/utils';
...
interface Props extends OceanFormProps {
      ...
      if (forward.value.handleValuesChange) {
        ...
        if (fields && fields.length > 0) {
          ...
          if (changedFields.length > 0) {
            ...
            forward.value.handleValuesChange(cloneDeep(await forward.value.formApi.getValues()), changedFields);
          }
        }
      }
