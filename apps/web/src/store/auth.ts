import type { Recordable, UserInfoType } from '@ocean/types';
...
import { resetAllStores, useAccessStore, useUserStore } from '@ocean/stores';
...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function authLogin(params: Recordable<any>, onSuccess?: () => Promise<void> | void) {
    ...
    let userInfo: null | UserInfoType = null;
    try {
      ...
      if (accessToken) {
        ...
        if (userInfo?.user?.remark) {
          ElNotification({
            message: `${$t('authentication.loginSuccessDesc')}:${userInfo?.user?.remark}`,
